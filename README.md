...
# Stock News Tracker
## What is it?
**Stock News Tracker** is a Django-based application completed as part of an assignment provided by [Zimran](https://zimran.io/ "Zimran's Homepage") company to assess the candidate applying for a Junior Python developer position.
## Built With
* Python
* Django
* Django REST framework
* Advanced Python Scheduler, aka APScheduler
* PostgreSQL
* Docker
## Description of the Assignment
### **Develop a microservice to parse and store Stock market news on the tickers once an hour to the database.**
...
* If you prefer to use an API platform, launch Postman/Insomnia on your laptop. The below-presented example will be used to explain how the software works.
![alt text](https://drive.google.com/uc?export=view&id=1C_majKmO275WJfjGHyDGFn7qzZiz49TJ "Insomnia")
#### Paste the following URLs in the field between "GET" dropdown and "Send" button and click to "Send". You should see data on the right side displayed in JSON format. These are the news for a specific ticker for the last 2 days.
...
##### Note: You may not immediately see data. In other words, data accessed via the above-presented API endpoints might return empty lists. It's because the APScheduler is set to run its jobs once an hour. You will see data in 1 hour. Grab your snacks and be patient!
