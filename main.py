...
class TicTacToe:
    def __init__(self, game):
        self.game = game
        self.field_image = self.get_scaled_image(path='resources/field.jpeg', res=[WIN_SIZE] * 2)
        self.O_image = self.get_scaled_image(path='resources/nought.png', res=[CELL_SIZE] * 2)
        # self.O_image2 = self.get_scaled_image(path='resources/круг.png', res=[CELL_SIZE] * 2)
        self.X_image = self.get_scaled_image(path='resources/cross.png', res=[CELL_SIZE] * 2)
        # self.X_image2 = self.get_scaled_image(path='resources/крестик.png', res=[CELL_SIZE] * 2)
        self.pen_sound = pg.mixer.Sound('resources/pen.wav')
        ...
        self.winner = None
        self.game_steps = 0
        self.font = pg.font.SysFont('Times New Roman', CELL_SIZE // 4, True)
    ...
    def draw_objects(self):
        for y, row in enumerate(self.game_array):
            for x, obj in enumerate(row):
                if obj != INF:
                    # self.game.screen.blit(self.X_image2 if obj else self.O_image2, vec2(x, y) * CELL_SIZE)
